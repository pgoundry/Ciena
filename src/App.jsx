...
// ... (Button, Card, TrendCard, ChatWidget, LoginPage components remain unchanged) ...
// ==========================================
// SHARED COMPONENTS (Buttons, Cards, Chat)
// ==========================================
...
const ChatWidget = ({ isOpen, onClose, context = "Finance" }) => {
  const [messages, setMessages] = useState([{ type: 'bot', text: `Hello! I'm your ${context} AI Coach. How can I help you today?` }]);
  ...
  const handleSend = (e) => {
    ...
    setInput('');
    setTimeout(() => setMessages(prev => [...prev, { type: 'bot', text: context === "WaveLogic" ? "WaveLogic 6 delivers 1.6Tb/s capacity per wavelength. It's designed to reduce power per bit by 50%." : "That's a great topic. In the finance vertical, latency and compliance are key." }]), 1000);
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 flex flex-col overflow-hidden animate-fade-in-up h-[500px]">
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 p-4 flex justify-between items-center text-white">
        <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-red-500 flex items-center justify-center"><Cpu className="w-4 h-4" /></div><span className="font-bold">{context} AI Coach</span></div>
        <button onClick={onClose} className="hover:bg-white/20 p-1 rounded-full"><X className="w-5 h-5" /></button>
      </div>
      <div className="flex-1 p-4 overflow-y-auto bg-gray-50 space-y-4">{messages.map((msg, idx) => (<div key={idx} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.type === 'user' ? 'bg-red-600 text-white' : 'bg-white border border-gray-200'}`}>{msg.text}</div></div>))}</div>
      <form onSubmit={handleSend} className="p-3 bg-white border-t border-gray-100 flex gap-2"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your response..." className="flex-1 px-4 py-2 bg-gray-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-red-500" /><button type="submit" className="p-2 bg-red-600 text-white rounded-full hover:bg-red-700 transition-colors"><Send className="w-4 h-4" /></button></form>
    </div>
  );
};
...
// ==========================================
// PAGE 7: FINANCE MICRO-LEARNING PAGE (NEW)
// ==========================================
      ...
      body: (
        <div className="space-y-6">
          ...
          <div className="rounded-xl overflow-hidden shadow-lg border border-gray-200 bg-black aspect-video relative group">
             {/* Placeholder for actual video source. Replace 'src' with your video URL */}
             <video 
               controls 
               className="w-full h-full object-cover"
               poster="https://images.unsplash.com/photo-1556761175-5973dc0f32e7?auto=format&fit=crop&q=80" // Placeholder thumbnail
             >
               <source src="https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4" type="video/mp4" />
               Your browser does not support the video tag.
             </video>
             
             {/* Overlay for context (optional, disappears on play if using custom controls) */}
             <div className="absolute inset-0 flex items-center justify-center pointer-events-none group-hover:bg-black/10 transition-colors">
               {/* Play button icon overlay could go here if using custom controls */}
             </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl p-6 text-white flex flex-col items-center text-center">
            ...
            <p className="text-base text-gray-300 max-w-2xl">
              The financial sector is the heartbeat of the global economy. For Ciena, it represents a high-value market driven by an uncompromising need for <span className="text-red-500 font-bold">speed</span>, <span className="text-red-500 font-bold">security</span>, and <span className="text-red-500 font-bold">reliability</span>.
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-3">
            <li className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-lg shadow-sm">
              ...
              <span><strong>Modernize Legacy Ops:</strong> Replace rigid, manual networks with programmable infrastructure.</span>
            </li>
            <li className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-lg shadow-sm">
              <CheckCircle className="w-5 h-5 text-green-500 shrink-0"/>
              <span><strong>Secure Data Mobility:</strong> Encrypt data in motion without adding latency.</span>
            </li>
          </ul>
        </div>
      )
    },
    {
      ...
      body: (
        <div className="space-y-6">
          ...
          {/* Optional: Second Video for Value Prop */}
          <div className="rounded-xl overflow-hidden shadow-sm border border-gray-200 aspect-[21/9] bg-gray-100 mb-4">
             ...
             >
               {/* Replace with a different video if needed */}
          ...
          <div className="grid gap-4">
            <div className="p-5 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl shadow-lg">
              <h4 className="font-bold text-lg mb-1">Adaptive Network™</h4>
              <p className="text-red-100 text-sm">We don't just build pipes. We build intelligent, automated networks that self-optimize for speed and reliability, ensuring your trading floor never goes dark.</p>
            </div>
            <div className="p-5 bg-white border border-gray-200 rounded-xl shadow-sm">
              <h4 className="font-bold text-gray-900 mb-1">WaveLogic 6</h4>
              <p className="text-gray-600 text-sm">The world's most advanced coherent optic. Delivers 1.6T capacity to handle your AI workloads while cutting power consumption in half.</p>
            </div>
          </div>
        </div>
      )
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900">Finance Vertical Fundamentals</h1>
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <Clock className="w-3 h-3"/> 5 min module
            </div>
          </div>
        </div>
        <div className="hidden md:flex items-center gap-2">
          <span className="text-sm font-medium text-gray-600">Step {step + 1} of {totalSteps}</span>
          <div className="w-32 h-2 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-red-600 transition-all duration-500" style={{ width: `${((step + 1) / totalSteps) * 100}%` }}></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border-t border-gray-200 p-6 sticky bottom-0">
        <div className="max-w-3xl mx-auto flex justify-between">
          <Button variant="outline" onClick={prevStep} disabled={step === 0} className={`${step === 0 ? 'opacity-0 pointer-events-none' : ''}`}>
            Previous
          </Button>
          {step === totalSteps - 1 ? (
            <Button onClick={onBack}>Finish Module</Button>
          ) : (
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="space-y-2">
          {['HIPAA', 'DORA', 'OSHA'].map((opt) => (
            <button 
              key={opt}
              onClick={() => setQ1(opt)}
              className={`w-full text-left px-4 py-3 rounded-lg border transition-all ${q1 === opt 
                ? (opt === 'DORA' ? 'bg-green-50 border-green-500 text-green-700' : 'bg-red-50 border-red-500 text-red-700') 
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="space-y-2">
          {[
            'It provides basic 10G connectivity', 
            'It delivers 1.6T capacity with lower power',
            'It is a software-only solution'
          ].map((opt) => (
            <button 
              key={opt}
              onClick={() => setQ2(opt)}
              className={`w-full text-left px-4 py-3 rounded-lg border transition-all ${q2 === opt 
                ? (opt.includes('1.6T') ? 'bg-green-50 border-green-500 text-green-700' : 'bg-red-50 border-red-500 text-red-700') 
                : 'border-gray-200 hover:bg-gray-50'}`}
            >
              {opt}
...
// ... (FinanceDigPage component remains unchanged) ...
// ==========================================
// PAGE 6: FINANCE DIG SHEET PAGE (NEW)
// ==========================================
  ...
  return (
    <div className="animate-fade-in bg-white min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
        ...
        {/* 2. Customer Challenges & Opportunities */}
        <section>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="bg-white p-6 border border-gray-200 rounded-xl shadow-sm">
              ...
              <ul className="space-y-3">
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  ...
                  <span><strong>Latency Lag:</strong> Microseconds lost in trading execution cost millions in revenue.</span>
                </li>
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  <X className="w-4 h-4 text-red-500 mt-1 shrink-0" />
                  <span><strong>Data Sovereignty Risk:</strong> Fear of compliance breaches when moving data to the cloud.</span>
                </li>
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  <X className="w-4 h-4 text-red-500 mt-1 shrink-0" />
                  <span><strong>Rigid Legacy Ops:</strong> Inability to spin up bandwidth quickly for new AI projects.</span>
                </li>
              </ul>
            </div>
            <div className="bg-white p-6 border border-gray-200 rounded-xl shadow-sm">
              ...
              <ul className="space-y-3">
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  ...
                  <span><strong>Low-Latency Optical:</strong> Position WaveLogic 6 for the fastest possible trading links.</span>
                </li>
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                  <span><strong>Encryption Everywhere:</strong> Sell WaveLogic Encryption for compliant, wire-speed security.</span>
                </li>
                <li className="flex items-start gap-2 text-gray-600 text-sm">
                  <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                  <span><strong>Bandwidth on Demand:</strong> Use MCP to show how they can automate capacity scaling.</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-blue-50 p-5 rounded-xl">
              <h3 className="font-bold text-blue-900 mb-2">Uncompromised Security</h3>
              <p className="text-sm text-blue-800">"Ciena delivers flight-data-recorder quality encryption at wire speed, ensuring you meet DORA compliance without sacrificing performance."</p>
            </div>
            <div className="bg-blue-50 p-5 rounded-xl">
              <h3 className="font-bold text-blue-900 mb-2">The Speed of Business</h3>
              <p className="text-sm text-blue-800">"Our programmable infrastructure shaves milliseconds off transaction times, giving your trading desks the competitive edge they need."</p>
            </div>
            <div className="bg-blue-50 p-5 rounded-xl">
              <h3 className="font-bold text-blue-900 mb-2">Future-Proof AI Ready</h3>
              <p className="text-sm text-blue-800">"Scale effortlessly to 800G and 1.6T to handle massive AI data sets without ripping and replacing your current fiber plant."</p>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             {["Waveserver", "6500 Family", "MCP Controller", "Encrypted Optics"].map((item, i) => (
               <div key={i} className="bg-white border border-gray-200 p-4 rounded-lg text-center font-semibold text-gray-700 hover:border-red-500 transition-colors cursor-default">
                 {item}
               </div>
             ))}
          </div>
        ...
        <section>
          ...
          <div className="bg-yellow-50 p-6 rounded-xl space-y-4">
             <div className="flex gap-3">
               ...
               <p className="text-gray-800 italic">"How is your current infrastructure handling the new operational resilience frameworks like DORA?"</p>
             </div>
             <div className="flex gap-3">
               <div className="mt-1.5 w-2 h-2 rounded-full bg-yellow-500 shrink-0"></div>
               <p className="text-gray-800 italic">"As you deploy more AI models, are you finding your current data center interconnects are becoming a bottleneck?"</p>
             </div>
             <div className="flex gap-3">
               <div className="mt-1.5 w-2 h-2 rounded-full bg-yellow-500 shrink-0"></div>
               <p className="text-gray-800 italic">"Are you looking at encrypting data in flight at Layer 1 to reduce the processing overhead on your routers?"</p>
             </div>
          </div>
        </section>
        ...
        <section className="border-t border-gray-200 pt-8">
          <div className="bg-gray-900 text-white rounded-xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <h3 className="text-2xl font-bold mb-2">Need Deeper Insights?</h3>
              <p className="text-gray-400">Access the full Finance Vertical toolkit for decks, videos, and more.</p>
            </div>
            <Button variant="primary" onClick={onBack} className="whitespace-nowrap">
              Open Full Toolkit
            </Button>
          </div>
        </section>
...
// ... (WaveLogicPage, TechnologiesPage, HomePage, PartnersPage components remain unchanged) ...
// Helper Icon for Challenges section (if needed)
...
// ... existing code for WaveLogicPage, TechnologiesPage, FinanceVerticalPage ...

// ==========================================
// PAGE 4: WAVELOGIC PAGE (NEW)
// ==========================================
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-24">
        ...
        <section id="verticals">
          <div className="flex items-center justify-between mb-8">
            ...
            <a href="#" className="text-red-600 font-semibold text-sm hover:underline flex items-center group">View All Verticals <ArrowRight className="w-4 h-4 ml-1 group-hover:translate-x-1 transition-transform" /></a>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-20">
        <section id="resources">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card 
              title="Finance DIG" 
              description="Your comprehensive Decision Intelligence Guide." 
              icon={FileText} 
              actionText="View DIG Sheet" 
              onClick={onNavigateToDig}
            />
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 selection:bg-red-100">
      ...
      {currentView === 'finance' && <FinanceVerticalPage onBack={() => navigateTo('home')} onNavigateToDig={() => navigateTo('financeDig')} onNavigateToMicroLearning={() => navigateTo('financeMicroLearning')} />}
